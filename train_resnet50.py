""" 
This script runs cheXNet training, expects the data to be in the form of TFRecords. Please refer to creating 
tf records if you have not already created from the raw images downloaded from NIH dataset.  
"""
...
# number of train tf record files
_NUM_TRAIN_FILES = 304

# number of validation tf record files
_NUM_VAL_FILES = 33
...
parser.add_argument(
    '--data_dir', type=str, default='/data/xraytfrecords',
    help='The directory where the input data is stored.')

parser.add_argument(
    '--model_dir', type=str, default='/workspace/output',
...
parser.add_argument(
    '--skip_eval', type=lambda x: (str(x).lower() in ['true','1', 'yes']), default=False,
    help='Defines if evaluation must be skipped while training'
)

parser.add_argument(
    '--write_weights', type=lambda x: (str(x).lower() in ['true','1', 'yes']), default=True,
...
def get_filenames(is_training, data_dir):
    """ Return filenames for dataset 
        is_training: if training,then returns the training dataset, else 
         returns the validation dataset.
        data_dir: location of chexnet tfrecords.
        
        Returns:
         An absolute list of tfrecord files.
    """

    if is_training:
        return [os.path.join(data_dir, 'train-%05d-of-%05d' % (i,_NUM_TRAIN_FILES)) for i in range(_NUM_TRAIN_FILES)] 
  
    else: 
        return [os.path.join(data_dir, 'val-%05d-of-%05d' % (i,_NUM_VAL_FILES))
            for i in range(_NUM_VAL_FILES)]


def record_parser(example_serialized, is_training, dtype):
    """ Parses and preprocesses an example proto containing a training example of an image - corrected."""

    keys_to_features = {
        'image/encoded': tf.io.FixedLenFeature([], tf.string, default_value=''),
        'image/label': tf.io.FixedLenSequenceFeature([],tf.int64, allow_missing=True),
        'image/filename': tf.io.FixedLenFeature([], tf.string, default_value='')}

    parsed = tf.parse_single_example(example_serialized, keys_to_features)
   
    # get the raw string to an image format and convert to the required dtype 
    ...
    #tf.print(filename,label,"\n \n tf filename and label:",summarize=-1)
...
def set_model_precision(FLAGS):
    # setup the model to use float16 if asked, however, at this point (with tf version 1.12.0), fp16 training with tf.keras isn't supported.
    # if you try to enable fp16 training, it may throw an error.
...
def main():
    ...
    # adding lr_reducer and model_checkpoint to callback list because default parameters disable to validation and hence these needs to be saved irrespective of the default parameters.
    #callbacks = [lr_reducer, model_checkpoint]
    callbacks = []

    # write weights
    if FLAGS.write_weights and not FLAGS.skip_eval:
    ...
    if FLAGS.enable_tensorboard:
        callbacks.append(TensorBoard(log_dir='./logs', histogram_freq=0, batch_size=FLAGS.batch_size))
    ...
    if not FLAGS.skip_eval:
        ...
        val_steps = (_NUM_VAL_IMAGES//FLAGS.batch_size)
    ...
    model.fit(
        train_input_dataset,
        steps_per_epoch= (_NUM_TRAINING_IMAGES//FLAGS.batch_size),
        epochs=FLAGS.epochs,
